...
async def update_contact_info(
    ...
    try:
        ...
        # Handle primary contact logic
        if contact_info.is_primary and contact_info.is_primary != existing_contact.is_primary:
            # Set all other contacts in the same category to non-primary
            category = contact_info.category or existing_contact.category
            await ContactInformation.find(
                ContactInformation.category == category
            ).update({"$set": {"is_primary": False}})
...
@router.delete("/contact-info/{contact_info_id}", status_code=status.HTTP_204_NO_CONTENT)
